...
async function release(checkOnly: boolean) {
  ...
  try {
    ...
    if (!errors.length) {
      ...
      const prefs={client:"codex",scope:"project",workspace:runtimeWork,orchestrator:{model:"inherit"},roles:{routine:{model:"gpt-5.6-luna",effort:"max"},high:{model:"gpt-5.6-terra",effort:"high"},hard:{model:"gpt-5.6-sol",effort:"high"},advisor:{model:"gpt-5.6-sol",effort:"high",readonly:true}}};
      ...
      if(runtimeCode!==0) fail(`extracted MCP server failed: ${runtimeErr}`); else if(init?.result?.serverInfo?.name!=="sol-advisor"||listed?.result?.tools?.length!==9||missing?.status!=="missing"||!saved?.saved||installed?.installed?.length!==4||removed?.removed?.length!==4) fail("extracted MCP server core-flow check failed");
    }
