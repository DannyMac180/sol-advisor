...
- Windows now validates `${PLUGIN_DATA}`, existing state files, and backups from owner and ACL SIDs instead of Bun's POSIX mode projection, while retaining POSIX mode checks on Linux and macOS.
