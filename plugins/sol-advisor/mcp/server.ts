...
type RuntimeEvidence={challenge:string;threadId:string;parentThreadId:null;latestEventAt:string;evidenceSource:"codex-rollout-inspector";executionContext:"parent";agentIdentifier:null;model:string;effort:string;observedRuntimeTier:"default"|"priority"|null;sandboxPolicyType:string;rawTokens:number;modelRounds:number;medianInputTokensPerRound:number|null;medianInputTokensFirst20:number|null;toolCalls:number;compactions:number};
type TargetRuntimeEvidence=Omit<RuntimeEvidence,"executionContext"|"agentIdentifier"|"parentThreadId">&{executionContext:"agent";agentIdentifier:string;parentThreadId:string};
...
const routeChallengeLifetimeMs=5*60_000, currentParentEvidenceMaxAgeMs=5*60_000, routeEvidenceFutureClockSkewMs=5_000;
...
function resolveRoute(args:any){
  ...
  const taskClass=args.taskClass as TaskClass;
  if(!(["routine","medium","hard","planning","review"] as TaskClass[]).includes(taskClass))throw new Error("taskClass is unsupported");const machineTier:MachineTier=args.machineTier??"default";if(machineTier!=="default"&&machineTier!=="fast")throw new Error("machineTier must be default or fast");
  ...
  const evidenceFields=["challenge","threadId","parentThreadId","latestEventAt","evidenceSource","executionContext","agentIdentifier","model","effort","observedRuntimeTier","sandboxPolicyType","rawTokens","modelRounds","medianInputTokensPerRound","medianInputTokensFirst20","toolCalls","compactions"];
  ...
  const route:{storageRole:RoleName}={routine:{storageRole:"routine"},medium:{storageRole:"high"},hard:{storageRole:"hard"},planning:{storageRole:"advisor"},review:{storageRole:"advisor"}}[taskClass];
  ...
  if(target&&target.parentThreadId!==current.threadId)return invalidated("target runtime evidence must be bound to the exact current parent thread","fresh_agent");
  const initialBlocked=(reason:string,placement:"parent"|"fresh_agent"="parent")=>{challenge.used=true;return blocked(reason,placement);};
...
const runtimeEvidenceProperties={challenge:str,threadId:str,parentThreadId:{type:["string","null"]},latestEventAt:str,evidenceSource:{const:"codex-rollout-inspector"},executionContext:{type:"string",enum:["parent","agent"]},agentIdentifier:{type:["string","null"]},model:str,effort:str,observedRuntimeTier:{type:["string","null"],enum:["default","priority",null]},sandboxPolicyType:str,rawTokens:nonnegativeInteger,modelRounds:nonnegativeInteger,medianInputTokensPerRound:{anyOf:[nonnegativeNumber,{type:"null"}]},medianInputTokensFirst20:{anyOf:[nonnegativeNumber,{type:"null"}]},toolCalls:nonnegativeInteger,compactions:nonnegativeInteger};
