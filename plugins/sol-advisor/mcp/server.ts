...
type RuntimeEvidence={evidenceSource:"codex-rollout-inspector";executionContext:"parent";agentIdentifier:null;model:string;effort:string;observedRuntimeTier:"default"|"priority"|null;sandboxPolicyType:string};
type TargetRuntimeEvidence={evidenceSource:"codex-rollout-inspector";executionContext:"agent";agentIdentifier:string;model:string;effort:string;observedRuntimeTier:"default"|"priority"|null;sandboxPolicyType:string};
function routeResult(taskClass:TaskClass,storageRole:RoleName,identifier:string,pref:RolePreference,machineTier:MachineTier,placement:"parent"|"fresh_agent",evidenceStatus:"blocked"|"spawn-required"|"verified",reason:string|null,current?:RuntimeEvidence,target?:TargetRuntimeEvidence){const selected=placement==="fresh_agent"&&target?target:current;return {taskClass,storageRole,agentIdentifier:identifier,configured:{model:pref.model,effort:pref.effort,readonly:pref.readonly===true},requestedMachineTier:machineTier,requestedRuntimeTier:machineTier==="fast"?"priority":"default",savedMachineTier:pref.machineTier,observedRuntimeTier:selected?.observedRuntimeTier??null,presentationLabel:machineTier==="fast"?"Fast":"Standard",currentRuntimeEvidence:current??null,targetRuntimeEvidence:target??null,evidenceStatus,blockedReason:reason,executionPlacement:placement,escalated:placement==="fresh_agent",forkContext:false,parallelism:1};}
...
function resolveRoute(args:any){
  ...
  assertSafeInput(args.currentRuntimeEvidence,"currentRuntimeEvidence");assertSafeInput(args.targetRuntimeEvidence,"targetRuntimeEvidence");assertSafeInput(args.fastOverride,"fastOverride");
  ...
  exactObject(args.currentRuntimeEvidence,["evidenceSource","executionContext","agentIdentifier","model","effort","observedRuntimeTier","sandboxPolicyType"],"currentRuntimeEvidence");
  if(args.targetRuntimeEvidence!==undefined)exactObject(args.targetRuntimeEvidence,["evidenceSource","executionContext","agentIdentifier","model","effort","observedRuntimeTier","sandboxPolicyType"],"targetRuntimeEvidence");
  ...
  const pref=prefs.roles[route.storageRole],identifier=prefs.client==="codex"?`sol_advisor_${route.storageRole}`:`sol-advisor-${route.storageRole}`,current=args.currentRuntimeEvidence as RuntimeEvidence,target=args.targetRuntimeEvidence as TargetRuntimeEvidence|undefined;
  ...
  if(machineTier==="fast"&&(!args.fastOverride||args.fastOverride.bounded!==true||args.fastOverride.oneRoute!==true))return blocked("Fast requires an explicit bounded one-route override");
  if(machineTier==="fast"&&(taskClass!=="routine"||pref.model!=="gpt-5.6-luna"||pref.effort!=="max"))return blocked("Fast is available only for the exact routine Luna/max route");
  ...
  if(!requiresFresh){const budget=machineTier==="fast"?{toolCalls:10,rawTokens:1_000_000,compactions:0,restartOnExpansion:"routine Luna/default"}:taskClass==="planning"?{toolCalls:25,rawTokens:2_500_000,compactions:0}:{toolCalls:50,rawTokens:5_000_000,compactions:1};return {...routeResult(taskClass,route.storageRole,identifier,pref,machineTier,"parent","verified",null,current,target),budget,outputLimit:{bytes:8192,lines:200},handoffTokenLimit:2000,contextPolicy:"batch independent reads; never carry raw logs or unchanged context"};}
  if(!target)return routeResult(taskClass,route.storageRole,identifier,pref,machineTier,"fresh_agent","spawn-required","spawn fresh exact target and supply targetRuntimeEvidence",current);
  if(!exact(target,true))return routeResult(taskClass,route.storageRole,identifier,pref,machineTier,"fresh_agent","blocked","target runtime evidence does not match the configured route",current,target);
...
export const tools = [
  ...
  {name:"resolve_route",description:"Fail-closed two-phase route resolution from current and target runtime evidence",inputSchema:objectSchema({taskClass:{type:"string",enum:["routine","medium","hard","planning","review"]},machineTier:{type:"string",enum:["default","fast"]},fastOverride:{type:"object",properties:{bounded:{const:true},oneRoute:{const:true}},required:["bounded","oneRoute"],additionalProperties:false},currentRuntimeEvidence:{type:"object",properties:{evidenceSource:{const:"codex-rollout-inspector"},executionContext:{const:"parent"},agentIdentifier:{type:"null"},model:str,effort:str,observedRuntimeTier:{type:["string","null"],enum:["default","priority",null]},sandboxPolicyType:str},required:["evidenceSource","executionContext","agentIdentifier","model","effort","observedRuntimeTier","sandboxPolicyType"],additionalProperties:false},targetRuntimeEvidence:{type:"object",properties:{evidenceSource:{const:"codex-rollout-inspector"},executionContext:{const:"agent"},agentIdentifier:str,model:str,effort:str,observedRuntimeTier:{type:["string","null"],enum:["default","priority",null]},sandboxPolicyType:str},required:["evidenceSource","executionContext","agentIdentifier","model","effort","observedRuntimeTier","sandboxPolicyType"],additionalProperties:false}},["taskClass","currentRuntimeEvidence"])},
...
export async function callTool(name:string,args:any={}) {
  const allowed:Record<string,string[]>={get_setup_status:[],get_preferences:[],save_preferences:["client","scope","workspace","orchestrator","roles","appTaskLane"],render_client_adapter:["workspace"],install_client_adapter:["workspace","confirmationToken","userScopeConfirmationToken","hardConsentToken"],uninstall_client_adapter:["confirmationToken"],resolve_route:["taskClass","machineTier","fastOverride","currentRuntimeEvidence","targetRuntimeEvidence"],validate_configuration:["workspace"],reset_configuration:["confirmationToken"]};
