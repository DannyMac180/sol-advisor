...
describe("route resolution",()=>{
 ...
 test("accepts the whole fresh shell-inspector object without translation",async()=>{
  ...
  ].map(record=>JSON.stringify(record)).join("\n")+"\n");const inspector=Bun.spawn(["sh",join(import.meta.dir,"..","scripts","inspect-agent-runtime.sh"),"--challenge",challenge,fixtureThread],{env:{...process.env,CODEX_HOME:runtimeHome},stdout:"pipe",stderr:"pipe"}),output=await new Response(inspector.stdout).text(),error=await new Response(inspector.stderr).text(),exit=await inspector.exited;if(exit!==0)throw new Error(error);const evidence=JSON.parse(output);expect(Object.keys(evidence).sort()).toEqual(["agentIdentifier","challenge","compactions","effort","evidenceSource","executionContext","latestEventAt","medianInputTokensFirst20","medianInputTokensPerRound","model","modelRounds","observedRuntimeTier","parentThreadId","rawTokens","sandboxPolicyType","threadId","toolCalls"].sort());expect(evidence.parentThreadId).toBeNull();const route:any=await callTool("resolve_route",{taskClass:"routine",challenge,currentRuntimeEvidence:evidence});expect(route).toMatchObject({evidenceStatus:"verified",executionPlacement:"parent",observedRuntimeTier:"default"});
 });
