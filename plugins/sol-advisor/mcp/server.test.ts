...
beforeEach(()=>{__resetDataPinForTests();evidenceSequence=0;root=realpathSync(mkdtempSync(join(tmpdir(),"sol-advisor-test-")));data=join(root,"data");workspace=join(root,"work");mkdirSync(data);chmodSync(data,0o700);mkdirSync(workspace);process.env.PLUGIN_DATA=data;});
...
describe("configuration",()=>{
 ...
 test("requires migrated hard consent and fresh exact runtime evidence",async()=>{
  const old:any=base();delete old.roles.hard;const profileKey=`codex:project:${workspace}`,legacy={schemaVersion:1,client:"codex",scope:"project",workspace,orchestrator:old.orchestrator,roles:old.roles,fallbackPolicy:"fail-closed",fallbacks:[],profileKey,createdAt:"x",updatedAt:"x",pluginVersion:"0.5.0"};
  writeFileSync(join(data,"config.json"),JSON.stringify({schemaVersion:1,activeProfile:profileKey,profiles:{[profileKey]:legacy}}));const preview:any=await callTool("render_client_adapter",{workspace});
  expect((await callTool("resolve_route",{taskClass:"hard"}) as any).evidenceStatus).toBe("blocked");
...
describe("route resolution",()=>{
 ...
 test("rejects expired, pre-issuance, same-thread, secret, and unknown runtime evidence",async()=>{
  await callTool("save_preferences",base());const clock=Date.now(),expiredChallenge=await challengeFor("routine");__setRouteChallengeNowForTests(()=>clock+301_000);await expect(callTool("resolve_route",{taskClass:"routine",challenge:expiredChallenge,currentRuntimeEvidence:parentEvidence(expiredChallenge,"gpt-5.6-luna","max")})).rejects.toThrow("expired");await challengeFor("routine");await expect(callTool("resolve_route",{taskClass:"routine",challenge:expiredChallenge,currentRuntimeEvidence:parentEvidence(expiredChallenge,"gpt-5.6-luna","max")})).rejects.toThrow("unknown");__setRouteChallengeNowForTests(undefined);
  const issued:any=await callTool("resolve_route",{taskClass:"routine"}),preIssuance=parentEvidence(issued.challenge,"gpt-5.6-luna","max","danger-full-access","default",{latestEventAt:new Date(Date.parse(issued.challengeIssuedAt)-1).toISOString()});await expect(callTool("resolve_route",{taskClass:"routine",challenge:issued.challenge,currentRuntimeEvidence:preIssuance})).rejects.toThrow("fresh-evidence");expect((await callTool("resolve_route",{taskClass:"routine",challenge:issued.challenge,currentRuntimeEvidence:{...preIssuance,latestEventAt:issued.challengeIssuedAt}}) as any).evidenceStatus).toBe("verified");
 ...
 test("guards Fast and keeps exact evidence fields distinct",async()=>{
  await callTool("save_preferences",base());expect((await callTool("resolve_route",{taskClass:"routine",machineTier:"fast"}) as any).blockedReason).toContain("bounded");
 ...
 test("keeps selected tiers, planning budget, and writable sandbox evidence distinct",async()=>{
  ...
  const hardChallenge=await challengeFor("hard"),fresh:any=await callTool("resolve_route",{taskClass:"hard",challenge:hardChallenge,currentRuntimeEvidence:parentEvidence(hardChallenge,"gpt-5.6-luna","max","danger-full-access","priority"),targetRuntimeEvidence:agentEvidence(hardChallenge,"sol_advisor_hard","gpt-5.6-sol","high","danger-full-access","default")});expect(fresh.observedRuntimeTier).toBe("default");const readonlyChallenge=await challengeFor("routine"),writableReadonly:any=await callTool("resolve_route",{taskClass:"routine",challenge:readonlyChallenge,currentRuntimeEvidence:parentEvidence(readonlyChallenge,"gpt-5.6-luna","max","read-only")});expect(writableReadonly.evidenceStatus).toBe("spawn-required");
 });
 test("accepts the whole fresh shell-inspector object without translation",async()=>{
  await callTool("save_preferences",base());const challenge=await challengeFor("routine"),fixtureThread="11111111-1111-7111-8111-111111111111",runtimeHome=join(root,"runtime-home"),rollout=join(runtimeHome,"sessions","2026","08","13",`rollout-2026-08-13T00-00-00-${fixtureThread}.jsonl`),at=new Date().toISOString();mkdirSync(join(runtimeHome,"sessions","2026","08","13"),{recursive:true});writeFileSync(rollout,[
   {timestamp:at,type:"session_meta",payload:{id:fixtureThread}},
   {timestamp:at,type:"turn_context",payload:{model:"gpt-5.6-luna",effort:"max",sandbox_policy:{type:"danger-full-access"}}},
   {timestamp:at,type:"event_msg",payload:{type:"thread_settings_applied",thread_settings:{service_tier:"default"}}},
   {timestamp:at,type:"event_msg",payload:{type:"token_count",info:{total_token_usage:{total_tokens:456},last_token_usage:{input_tokens:30}}}},
   {timestamp:at,type:"response_item",payload:{type:"custom_tool_call"}}
  ].map(record=>JSON.stringify(record)).join("\n")+"\n");const inspector=Bun.spawn(["sh",join(import.meta.dir,"..","scripts","inspect-agent-runtime.sh"),"--challenge",challenge,fixtureThread],{env:{...process.env,CODEX_HOME:runtimeHome},stdout:"pipe",stderr:"pipe"}),output=await new Response(inspector.stdout).text();expect(await inspector.exited).toBe(0);const evidence=JSON.parse(output);expect(Object.keys(evidence).sort()).toEqual(["agentIdentifier","challenge","compactions","effort","evidenceSource","executionContext","latestEventAt","medianInputTokensFirst20","medianInputTokensPerRound","model","modelRounds","observedRuntimeTier","rawTokens","sandboxPolicyType","threadId","toolCalls"].sort());const route:any=await callTool("resolve_route",{taskClass:"routine",challenge,currentRuntimeEvidence:evidence});expect(route).toMatchObject({evidenceStatus:"verified",executionPlacement:"parent",observedRuntimeTier:"default"});
 });
});
