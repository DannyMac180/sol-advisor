...
- `${PLUGIN_DATA}` must be an absolute existing private directory: on Linux and macOS it may have no group/world permission bits; on Windows its owner must be the current user, SYSTEM, or Administrators, and allow ACEs are restricted to those principals plus CREATOR OWNER and OWNER RIGHTS. The exact machine-local `CodexSandboxUsers` group is also accepted when it exists, but only while every allow ACE for its resolved SID stays within read/execute access; lookup failure adds no exception, and modify, write, delete, permission-change, ownership, or full-control access remains forbidden. A null DACL is always rejected. Existing Windows state files (`config.json`, `managed-files.json`, and `transaction.json`) are checked with the same owner/DACL policy before they are read, and existing `.bak` files are checked before the backup store is reused; newly created files inherit from the validated private parent and generated backups are verified after creation. The directory may never be the filesystem root, home directory, plugin root, or a path with symlink ancestors. Its realpath/device/inode are pinned for the server process; Sol Advisor never changes permissions on the host-supplied root.
